...
export function emailShell(params: {
  ...
  const preview = params.preview ? escapeHtml(params.preview) : undefined;
...
export function ctaButton(href: string, label: string) {
  return `<p style="margin:28px 0 0;">
    <a href="${escapeHtml(href)}" style="display:inline-block;padding:12px 20px;border-radius:12px;background:linear-gradient(90deg,#22d3ee,#14b8a6);color:#041016;text-decoration:none;font-weight:700;">
      ${escapeHtml(label)}
    </a>
  </p>`;
}
