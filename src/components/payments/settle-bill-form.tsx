        ...
        <CardContent>
              ...
              startTransition(async () => {
                ...
                toast.success(
                  response.data?.validation.overallMatch
                    ? "Receipt matched — bill marked paid"
                    : "Could not auto-confirm — sent for admin review"
                );
                router.refresh();
              });
            }}
          >
            <div className="space-y-2">
              ...
              >
                ...
                <p className="text-sm font-medium">
                  {fileName ?? "Upload transfer receipt screenshot"}
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  GCash, Maya, or bank transfer receipt · PNG, JPG, or WEBP up to
                  8MB
                </p>
                <input
                  id="receipt"
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="referenceNumber">
                  Reference number{" "}
                  <span className="text-muted-foreground">(if OCR misses it)</span>
                </Label>
                <Input
                  id="referenceNumber"
                  name="referenceNumber"
                  placeholder="Paste from your transfer receipt"
                />
              </div>
              <div className="space-y-2">
            ...
            {!ocrLive && (
              <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.02] px-3 py-2">
                ...
                <Label htmlFor="forceMismatch" className="text-xs text-muted-foreground">
                  Demo: force OCR mismatch (mock provider only)
                </Label>
              </div>
            )}
            ...
            <p className="text-xs text-muted-foreground">
              {ocrLive
                ? "OCR.space extracts amount and reference only. Matching both auto-confirms payment. The receipt is compressed before saving to storage."
                : "Upload a receipt. Amount and reference are checked against this bill. The image is compressed before storage."}
            </p>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              >
                {pending
                  ? ocrLive
                    ? "Reading receipt with OCR.space..."
                    : "Validating receipt..."
